function [stiffkPhiPhi,Phi] = gStiffnessPhiPhi1D(PHTelem,Basis,Mesh,Fract,fenerg,tdisp,degFunDeriv)
ngaussX = Mesh.nGauss;
    ...
    for i=1:length(PHTelem{indexPatch})
        if isempty(PHTelem{indexPatch}(i).children)            
            ...
            for ii=1:ngaussX                
                    ...
                    localkPhiPhi = localkPhiPhi + (((Fract.cenerg/Fract.constl)...
                        + ddphi*senerg)).*(Basis.shape{indexPatch}{i}(kgauss,:)'...
                        *Basis.shape{indexPatch}{i}(kgauss,:)).*Basis.volume{indexPatch}{i}(kgauss);   
