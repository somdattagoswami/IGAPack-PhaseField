...
cla(plot1)
plotMesh2D(PHTelem,controlPts,geometry,0)
axis equal
title('Intial Mesh');
...
[shape,dgdx,d2gdx2,volume,gaussCord,numberElements]=cartdev2nd(PHTelem,controlPts,geometry);
...
for istep = 1:Integ.nstep+1
    ...
    while (normInnerStep > geometry.toler)
        ...
        tic
        tic
        [stiffUU,elemRef] = PF.StiffUU(PHTelem,sizeBasis,numberElements,dgdx,shape,Fract,Mater,volume,tdisp,geometry);
        toc
        ...
        tic
        [stiffPhiPhi,RHSPhi] = PF.StiffPhiPhi(PHTelem,sizeBasis,numberElements,dgdx,d2gdx2,shape,Fract,Mater,volume,fenerg,geometry,tdisp);
        solPhiOld = solPhi;
