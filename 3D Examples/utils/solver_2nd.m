...
[shape,dgdx,volume,gaussCoord,sctrxElem,numberElements] = cartdev3D(PHTelem,controlPts,geometry);
...
for istep = 1:Integ.nstep+1
    ...
    while (normInnerStep > geometry.toler)
        ...
        parfor iBlock = 1:numBlocks
            lengthBlock = length(blockIndex{iBlock});
            [stiffPhiPhiCell,RHSPhiCell,fenergSlice{iBlock}] = PF.StiffPhiPhi(lengthBlock,sctrxElemSlice{iBlock},sizeBasis,dgdxSlice{iBlock},...
                shapeSlice{iBlock},volumeSlice{iBlock},Fract,Mater,geometry,fenergSlice{iBlock},tdisp(1:3*sizeBasis),solPhi);
        ...
        if refFlag
            tic
            clear stiffUU
            [PHTelem{1},controlPts{1},dimBasis,solPhiPatch{1},numberElements] = refineElemProjGradedIso3D(elemRef,PHTelem{1},controlPts{1},geometry,dimBasis,solPhiPatch{1},numberElements);
            [PHTelem,sizeBasis] = zipConforming3D(PHTelem,dimBasis,geometry);
            [dirichlet] = PF.Boundary(PHTelem,geometry);
            [shape,dgdx,volume,gaussCoord,sctrxElem] = cartdev3D(PHTelem,controlPts,geometry);
            [fenerg,elemIndex] = PF.History(gaussCoord,Fract,PHTelem,geometry,numberElements);
            clear gaussCoord
