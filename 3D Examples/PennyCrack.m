% Script for cube with a penny shaped crack in the center
...
Input = @() Input_penny;
PF.Geometry = @(geometry)initMeshPenny(geometry);
PF.Boundary = @(PHTelem,geometry,controlPts)initialBC_penny(PHTelem,geometry);
PF.History = @(gaussCoord,Fract,PHTelem,geometry,numberElements)history_penny(gaussCoord,Fract,PHTelem,geometry,numberElements);
PF.Trac = @(stiffUU,tdisp,Integ,dirichlet,file_name)compTreacTension(stiffUU,tdisp,Integ,dirichlet,file_name);

file_name = 'FD-penny.txt';
